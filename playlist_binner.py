...
outkeys = ['name', 'num_splits/num_tracks', 'idx', 'file', 'pid', 'modified_at', 'collaborative', 'num_albums', 'num_followers']
...
def get_playlist_info(cur_file, playlist, pl_idx):
    ...
    return {'name': cur_name, 'idx': pl_idx, 'file': cur_file, 'pid': cur_pid, 'num_splits/num_tracks': cur_tracks,
    ...
    with open(cpath, 'r') as f:
        ...
        for pl_idx, playlist in enumerate(j['playlists']):
            ...
            if cur_bin not in track_bins.keys():
                track_bins[cur_bin] = []
            track_bins[cur_bin].append((playlist['num_splits/num_tracks'], playlist['pid'], cur_info))

for cur_bin in track_bins.keys():
    cur_f = f"num_splits/num_tracks-{cur_bin}.csv"
